...
Windows 알림 영역(시스템 트레이)에서 Codex의 7일 사용량을 확인하는 작은 앱입니다. `--` 아이콘을 클릭하면 사용량, 초기화 시각, 초기화까지 남은 시간을 볼 수 있습니다.

이 앱은 작업표시줄의 시간·날짜 영역 위에 창을 띄우지 않습니다. 표준 Windows 트레이 아이콘이므로 Windows 설정의 **기타 시스템 트레이 아이콘**에서 이 앱을 켤지 여부는 사용자가 결정합니다. 아이콘은 굵은 Codex 녹색으로 잔여 숫자 또는 `--`만 크게 표시하고, 아이콘 위에 마우스를 올리면 `73% · 7일 잔여`처럼 정확한 잔여 비율이 표시됩니다.
...
Windows 알림 영역에서 `--` 아이콘을 찾으세요. 보이지 않으면 Windows 설정의 **개인 설정 > 작업 표시줄 > 기타 시스템 트레이 아이콘**에서 `Codex WeekUsage Tray`를 켭니다. 7일 제한 버킷을 읽지 못한 경우에도 팝업에서 **로그인** 또는 **다시 확인**을 선택할 수 있습니다.

## 동작

- `account/rateLimits/updated` 알림을 받으면 즉시 갱신합니다.
- 알림 누락에 대비해 5초마다 한 번만 다시 확인합니다. 1분 단위 폴링을 사용하지 않습니다.
- 사용량을 읽기 전에는 `--`, 읽은 뒤에는 `73`처럼 잔여 비율만 크게 표시한 트레이 아이콘을 클릭하면 7일 잔여·사용 비율, 초기화 시각, 남은 시간이 나타납니다.
- 팝업의 **새로 고침**을 누르면 즉시 다시 조회합니다.
- 로그인이 없으면 팝업의 **로그인** 또는 우클릭 메뉴의 **Codex 로그인**이 Codex가 제공한 브라우저 인증을 시작합니다. 브라우저가 열려 있는 상태에서 다시 누르면 기존 시도를 취소하고 새 OAuth 페이지를 엽니다.
