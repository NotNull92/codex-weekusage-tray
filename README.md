...
Windows는 임의의 텍스트를 작업표시줄에 직접 추가하는 공식 API를 제공하지 않습니다. 이 앱은 기본 작업표시줄 위에 작은 클릭 가능 창을 배치하는 방식으로 `7D 73%` 같은 카운터를 표시합니다.
...
작업표시줄 오른쪽의 `7D --`가 `7D <잔여>%`로 바뀌면 준비된 것입니다. 7일 제한 버킷을 계정에서 제공하지 않으면 팝업에 안내 문구를 표시합니다.
